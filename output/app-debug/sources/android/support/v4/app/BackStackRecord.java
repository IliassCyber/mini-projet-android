...
final class BackStackRecord extends FragmentTransaction implements FragmentManager.BackStackEntry, Runnable {
    ...
    public void run() {
        ...
        if (!this.mAddToBackStack || this.mIndex >= 0) {
            ...
            fragmentManagerImpl.moveToState(fragmentManagerImpl.mCurState, transition, transitionStyle, true);
            if (this.mAddToBackStack) {
    ...
    public TransitionState popFromBackStack(boolean doStateMove, TransitionState state, SparseArray<Fragment> firstOutFragments, SparseArray<Fragment> lastInFragments) {
        ...
        if (doStateMove) {
            FragmentManagerImpl fragmentManagerImpl = this.mManager;
            fragmentManagerImpl.moveToState(fragmentManagerImpl.mCurState, FragmentManagerImpl.reverseTransit(transition), transitionStyle, true);
            state2 = null;
        }
        int i2 = this.mIndex;
        if (i2 >= 0) {
            this.mManager.freeBackStackIndex(i2);
            this.mIndex = -1;
        }
        return state2;
    }
    ...
    private boolean configureTransitions(int containerId, TransitionState state, boolean isBack, SparseArray<Fragment> firstOutFragments, SparseArray<Fragment> lastInFragments) {
        ...
        Fragment inFragment;
        ArrayList<View> sharedElementTargets2;
        ...
        final Fragment inFragment2 = lastInFragments.get(containerId);
        Fragment outFragment = firstOutFragments.get(containerId);
        Object enterTransition3 = getEnterTransition(inFragment2, isBack);
        Object sharedElementTransition2 = getSharedElementTransition(inFragment2, outFragment, isBack);
        Object exitTransition2 = getExitTransition(outFragment, isBack);
        if (enterTransition3 == null && sharedElementTransition2 == null && exitTransition2 == null) {
            return false;
        }
        ArrayList<View> sharedElementTargets3 = new ArrayList<>();
        if (sharedElementTransition2 == null) {
            sharedElementTargets = sharedElementTargets3;
            enterTransition = enterTransition3;
            namedViews = null;
        } else {
            ArrayMap<String, View> namedViews2 = remapSharedElements(state, outFragment, isBack);
            sharedElementTargets3.add(state.nonExistentView);
            sharedElementTargets3.addAll(namedViews2.values());
            SharedElementCallback callback = isBack ? outFragment.mEnterTransitionCallback : inFragment2.mEnterTransitionCallback;
            if (callback == null) {
                sharedElementTargets = sharedElementTargets3;
                enterTransition = enterTransition3;
            } else {
                sharedElementTargets = sharedElementTargets3;
                ArrayList<String> names = new ArrayList<>(namedViews2.keySet());
                enterTransition = enterTransition3;
                ArrayList<View> views = new ArrayList<>(namedViews2.values());
                callback.onSharedElementStart(names, views, null);
            }
            namedViews = namedViews2;
        }
        ArrayList<View> exitingViews2 = new ArrayList<>();
        Object exitTransition3 = captureExitingViews(exitTransition2, outFragment, exitingViews2, namedViews, state.nonExistentView);
        ArrayList<String> arrayList = this.mSharedElementTargetNames;
        if (arrayList != null && namedViews != null) {
            View epicenterView = namedViews.get(arrayList.get(0));
            if (epicenterView != null) {
                if (exitTransition3 != null) {
                    FragmentTransitionCompat21.setEpicenter(exitTransition3, epicenterView);
                }
                if (sharedElementTransition2 != null) {
                    FragmentTransitionCompat21.setEpicenter(sharedElementTransition2, epicenterView);
                }
            }
        }
        ...
        if (sharedElementTransition2 == null) {
            ...
            inFragment = inFragment2;
            sharedElementTargets2 = sharedElementTargets;
            ...
            enterTransition2 = enterTransition;
            inFragment = inFragment2;
            prepareSharedElementTransition(state, sceneRoot, sharedElementTransition2, inFragment2, outFragment, isBack, sharedElementTargets2);
        }
        ...
        Object transition = FragmentTransitionCompat21.mergeTransitions(enterTransition2, exitTransition, sharedElementTransition3, allowOverlap);
        if (transition != null) {
            z = false;
            FragmentTransitionCompat21.addTransitionTargets(enterTransition2, sharedElementTransition3, sceneRoot, viewRetriever, state.nonExistentView, state.enteringEpicenterView, state.nameOverrides, enteringViews, renamedViews, sharedElementTargets2);
            ...
            FragmentTransitionCompat21.cleanupTransitions(sceneRoot, state.nonExistentView, enterTransition2, enteringViews, exitTransition, exitingViews, sharedElementTransition3, sharedElementTargets2, transition, state.hiddenFragmentViews, renamedViews);
        } else {
            z = false;
        }
        if (transition != null) {
    ...
    private void prepareSharedElementTransition(final TransitionState state, final View sceneRoot, final Object sharedElementTransition, final Fragment inFragment, final Fragment outFragment, final boolean isBack, final ArrayList<View> sharedElementTargets) {
        sceneRoot.getViewTreeObserver().addOnPreDrawListener(new ViewTreeObserver.OnPreDrawListener() { // from class: android.support.v4.app.BackStackRecord.2
            ...
            public boolean onPreDraw() {
                ...
                if (obj != null) {
                    FragmentTransitionCompat21.removeTargets(obj, sharedElementTargets);
