...
class MaterialProgressDrawable extends Drawable implements Animatable {
    ...
    @IntDef({Connections.DURATION_INDEFINITE, 1})
    ...
    private void setupAnimators() {
        ...
        Animation animation = new Animation() { // from class: android.support.v4.widget.MaterialProgressDrawable.1
            ...
            public void applyTransformation(float interpolatedTime, Transformation t) {
                ...
                float minProgressArc = (float) Math.toRadians(strokeWidth / centerRadius);
                ...
                float endTrim = (MaterialProgressDrawable.START_CURVE_INTERPOLATOR.getInterpolation(interpolatedTime) * minArc) + startingEndTrim;
                ring.setEndTrim(endTrim);
                float startTrim = (MaterialProgressDrawable.END_CURVE_INTERPOLATOR.getInterpolation(interpolatedTime) * MaterialProgressDrawable.MAX_PROGRESS_ARC) + startingTrim;
                ring.setStartTrim(startTrim);
                float rotation = (0.25f * interpolatedTime) + startingRotation;
                ring.setRotation(rotation);
                float groupRotation = (144.0f * interpolatedTime) + ((MaterialProgressDrawable.this.mRotationCount / 5.0f) * 720.0f);
    ...
    private static class Ring {
        ...
        public void draw(Canvas c, Rect bounds) {
            ...
            if (this.mAlpha < 255) {
                ...
                this.mCirclePaint.setAlpha(MotionEventCompat.ACTION_MASK - this.mAlpha);
        ...
        private void drawTriangle(Canvas c, float startAngle, float sweepAngle, Rect bounds) {
            if (this.mShowArrow) {
                ...
                double dExactCenterX = bounds.exactCenterX();
                Double.isNaN(dExactCenterX);
                float x = (float) (dCos + dExactCenterX);
                double dSin = this.mRingCenterRadius * Math.sin(0.0d);
                double dExactCenterY = bounds.exactCenterY();
                Double.isNaN(dExactCenterY);
                float y = (float) (dSin + dExactCenterY);
                this.mArrow.moveTo(0.0f, 0.0f);
                this.mArrow.lineTo(this.mArrowWidth * this.mArrowScale, 0.0f);
                Path path2 = this.mArrow;
                float f = this.mArrowWidth;
                float f2 = this.mArrowScale;
                path2.lineTo((f * f2) / 2.0f, this.mArrowHeight * f2);
