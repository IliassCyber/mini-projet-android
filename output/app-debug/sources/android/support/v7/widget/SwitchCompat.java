...
public class SwitchCompat extends CompoundButton {
    ...
    public void setSwitchTextAppearance(Context context, int resid) {
        TypedArray appearance = context.obtainStyledAttributes(resid, android.support.v7.appcompat.R.styleable.SwitchCompatTextAppearance);
        ColorStateList colors = appearance.getColorStateList(android.support.v7.appcompat.R.styleable.SwitchCompatTextAppearance_android_textColor);
        if (colors != null) {
            this.mTextColors = colors;
        } else {
            this.mTextColors = getTextColors();
        }
        int ts = appearance.getDimensionPixelSize(android.support.v7.appcompat.R.styleable.SwitchCompatTextAppearance_android_textSize, 0);
        if (ts != 0 && ts != this.mTextPaint.getTextSize()) {
            this.mTextPaint.setTextSize(ts);
            requestLayout();
        }
        boolean allCaps = appearance.getBoolean(android.support.v7.appcompat.R.styleable.SwitchCompatTextAppearance_textAllCaps, false);
        if (allCaps) {
    ...
    private Layout makeLayout(CharSequence text) {
        ...
        return new StaticLayout(transformed, this.mTextPaint, (int) Math.ceil(Layout.getDesiredWidth(transformed, r3)), Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, true);
    }
