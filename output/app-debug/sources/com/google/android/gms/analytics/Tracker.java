...
public class Tracker extends zzd {
    ...
    private class zza extends zzd implements GoogleAnalytics.zza {
        ...
        public void zzn(Activity activity) {
            ...
            if (this.zzIV) {
                ...
                tracker.set("&cd", tracker.zzIM != null ? Tracker.this.zzIM.zzq(activity) : activity.getClass().getCanonicalName());
    ...
    public void send(Map<String, String> map) {
        ...
        if (TextUtils.isEmpty(str)) {
            zzhQ().zzg(map2, "Missing hit type parameter");
            return;
        }
        final String str2 = map2.get("tid");
        if (TextUtils.isEmpty(str2)) {
            zzhQ().zzg(map2, "Missing tracking id parameter");
            return;
        }
        ...
        zzhS().zze(new Runnable() { // from class: com.google.android.gms.analytics.Tracker.1
            ...
            public void run() {
                ...
                zzam.zzc(map2, "cid", Tracker.this.zzhg().getClientId());
                ...
                if (zZzhp) {
                    zzam.zzb((Map<String, String>) map2, "ate", zzaVarZzhW.zzhy());
                    zzam.zzb((Map<String, String>) map2, "adid", zzaVarZzhW.zzhC());
                } else {
                    map2.remove("ate");
                    map2.remove("adid");
                }
                zznx zznxVarZzix = Tracker.this.zzhX().zzix();
                zzam.zzb((Map<String, String>) map2, "an", zznxVarZzix.zzjL());
                zzam.zzb((Map<String, String>) map2, "av", zznxVarZzix.zzjN());
                zzam.zzb((Map<String, String>) map2, "aid", zznxVarZzix.zzsK());
                zzam.zzb((Map<String, String>) map2, "aiid", zznxVarZzix.zzwi());
                map2.put("v", "1");
                map2.put("_v", zze.zzJB);
                zzam.zzb((Map<String, String>) map2, "ul", Tracker.this.zzhY().zzjE().getLanguage());
                zzam.zzb((Map<String, String>) map2, "sr", Tracker.this.zzhY().zzjF());
                if (!(str.equals("transaction") || str.equals("item")) && !Tracker.this.zzIJ.zzkb()) {
                    Tracker.this.zzhQ().zzg(map2, "Too many hits sent too quickly, rate limiting invoked");
                    return;
                }
                long jZzbj = zzam.zzbj((String) map2.get("ht"));
                if (jZzbj == 0) {
                    jZzbj = jCurrentTimeMillis;
                }
                long j = jZzbj;
                if (zIsDryRunEnabled) {
                    Tracker.this.zzhQ().zzc("Dry run enabled. Would have sent hit", new zzab(Tracker.this, map2, j, zZze));
                    return;
                }
                String str4 = (String) map2.get("cid");
                HashMap map3 = new HashMap();
                zzam.zza(map3, "uid", (Map<String, String>) map2);
                zzam.zza(map3, "an", (Map<String, String>) map2);
                zzam.zza(map3, "aid", (Map<String, String>) map2);
                zzam.zza(map3, "av", (Map<String, String>) map2);
                zzam.zza(map3, "aiid", (Map<String, String>) map2);
                map2.put("_s", String.valueOf(Tracker.this.zzhl().zza(new zzh(0L, str4, str2, !TextUtils.isEmpty((CharSequence) map2.get("adid")), 0L, map3))));
                Tracker.this.zzhl().zza(new zzab(Tracker.this, map2, j, zZze));
            }
        });
    }
